...
export function AvatarMenu({ me, isPlatformRoot = false }: { me: User | null; isPlatformRoot?: boolean }) {
  ...
  async function signOut() {
    setSigningOut(true);
    try {
      /* the flow itself lives in lib/signOut — the sessions table's
         "this device" row offers the same act, and two copies drift */
      await signOutThisDevice(locale);
    } finally {
      setSigningOut(false);
    }
  }
            ...
            <div className="flex gap-1">
              {(["dark", "light"] as const).map((option) => (
                <button
                  ...
                  className={`tap flex flex-1 items-center justify-center rounded-lg py-1.5 text-center text-xs leading-control transition-colors ${
                    theme === option
