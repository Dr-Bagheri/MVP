...
Parallel Codex sessions: **steward** (architecture, coordination, verification,
...
- **Windows/PS 5.1 hazard: never write source files with `Set-Content` /
  ...
  list is written out above. One expected hit in `AGENTS.md` is
...
- 2026-08-13 (decisions surface built + a possible M5 violation): 0028 at
  ...
  includes anthropic models → **PRIORITY CHECK: no-Codex filter (M5 user
  directive) may be unapplied — verify, fix, negative-space test, purge
  Codex from the ranking**. Logger test verified-red + two self-catches
...
- 2026-08-13 (0029 + born-tested card): 0028's rename REVERSED cleanly
  ...
  confirm-wiring + Codex-filter check; FE2 picker + responsive audit.
- 2026-08-13 (BACKEND CORE OF MILESTONE 3 SHUT, 411 tests): live loop
  ...
  **decision-FIRST ordering carries the guarantee** (replay refused
  ...
  B1's Codex-filter report, B2 dispatcher/purge close, FE2 picker +
...
- 2026-08-13 (FE2 audit round 1 accepted — AUDIT-responsive-dark.md):
  ...
  + en<1440.
- 2026-08-13 (NO-Codex FILTER: never existed — now structural): live
  ...
  with Codex deliberately IN the mock, ranking cleaned (gpt-5-mini +
  gemini-3.6-pro replace the two Codex entries). 304 served. Root cause:
...
- 2026-08-13 (/v1/me live + third instance confirmed): /me shipped with
  ...
  declared-served-never-written instance (last_seen_at, no-Codex
