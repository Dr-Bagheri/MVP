...
const REQUIRED_ROUTES: [method: string, path: string, why: string][] = [
  ...
  ["GET", "/v1/connectors", "M30/D29 — the person's own connections"],
  ["POST", "/v1/connectors/:provider/connect", "2026-09-06 — a pasted credential (Telegram, WhatsApp, an MCP server), vouched for by the provider before it is stored"],
  ["POST", "/v1/connectors/:provider/actions/:action", "2026-09-06 — a connector action, reached only from a client tool behind the consent card"],
  ["GET", "/v1/connectors/:provider/:source", "M47 — one listing door for every provider's sources"],
  ...
  // the rooms (db/0164) — the agents chat separately from the assistant, and
