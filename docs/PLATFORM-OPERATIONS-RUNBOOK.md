...
| **M5 env rung** | `WORKER_SUMMARY_MODEL` in `core.env` | The operator's model fallback (owner pref → org first choice → THIS → skip). **NOT SET on the production server** — checked 2026-09-18 by listing the env's names, and this row previously claimed it had been set on 2026-08-27 (`google/gemini-2.5-flash`) after the P2 workflow acceptance found it empty. Either it never landed or it was lost in a later deploy; what is certain is that production runs today with no env rung at all, which is why the 2026-09-18 narrowing had to decide what an org whose whole allow-list is unservable gets (answer: read as no curation, see SPEC). A value put here must be one `OFFERED_MODELS` names, or it is an absent rung that looks like a setting. |
