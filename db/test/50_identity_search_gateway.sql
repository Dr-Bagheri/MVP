...
insert into argued_deletes (name) values
  ...
  ('chat_channel_member'),
  -- 0189: REMOVING YOUR OWN REACTION. Pressing an emoji twice is taking it
  -- back, and the primary key is what makes that a delete rather than a
  -- toggle column somebody maintains. Scoped to the actor by policy, so this
  -- grant cannot reach anybody else's opinion.
  ('chat_reaction'),
  -- 0189: WITHDRAWING AN INVITATION YOU SENT. The invitee's "no" is the
  -- `declined` STATE and never a delete -- a record that says nothing
  ...
  -- 0212: UNLINKING YOUR OWN TELEGRAM ACCOUNT, and cancelling the code that
  -- would link it. Both are the personal shape at its narrowest: the policy
  -- scopes each to `user_id = echo.actor_id()`, so this grant reaches exactly
  -- one person's rows -- their own -- and nobody, including an org owner, can
  -- reach anybody else's. The INSERT half is deliberately NOT granted: a link
  -- exists only because a code was redeemed through the definer door, which
  -- is the whole reason the code proves anything.
  ('telegram_identity'), ('telegram_link_code'),
  ('meeting_attendee'),
...
select t.ok(
  ...
  'core/''s own role deletes exactly the argued list: a note author''s own note (0079), a task''s checklist lines and its assignee rows (0144), a label and a card''s wearing of one (0147), a meeting''s attached document (0159), a meeting''s decisions and action items (0160), a person''s membership of a project (0181), a person''s place on a meeting (0202), a person''s membership of a channel (0184), a person''s own reaction and an inviter''s withdrawn invitation (0189), a project itself (0191), a person''s own Telegram link and its code (0212) — every other product row is echo_purge''s alone');
