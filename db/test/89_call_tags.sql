...
-- row bob can SEE but not outrank — a private row would refuse by matching
-- nothing, which raises no error and would pass t.denied vacuously
reset role;
...
-- bob (member) on the ADMIN's org-scoped call: visible, so the GUARD is
-- what refuses — the discriminating case, not an invisible-row no-op
select t.denied(
  $$update echo.call set tags = array['نفوذ']
     where id = 'c8600000-0000-4000-8000-000000000086'$$,
  'a member cannot tag a record whose owner outranks them — 0077, unchanged');
